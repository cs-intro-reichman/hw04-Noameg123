...
public class ArrCharOps {
    ...
    int i = arr.length - 1; // מתחילים מהתו האחרון במערך
    while (i >= 0) {
        if (arr[i] == ch) {
            return i; // מצאנו את התו, מחזירים את המיקום
        }
        i--;
    }
    return -1; // לא מצאנו את התו כלל
    }
    ...
    */
    ...
    public static long hashCode(char[] arr) {
        ...
        long result = 0;
        for (int i= 0 ; i<arr.length; i++){
            result = result + arr[i]* (long)(Math.pow(7, n- (i+1)));
        }
        return result;
    }
